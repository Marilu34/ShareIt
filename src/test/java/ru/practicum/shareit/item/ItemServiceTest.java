...
class ItemServiceTest {
    ...
    @Test
    void create() {
        ItemDto actual = itemService.createItem(1L, itemDtoList.get(1));
        ...
        //user not exists
        assertThrows(NotFoundException.class, () -> itemService.createItem(999L, itemDtoList.get(2)));

        //empty fields of dto
        assertThrows(ValidationException.class,
                () -> itemService.createItem(2L, itemDtoList.get(3).toBuilder().name(null).build()));
    }

    @Test
    void update() {
        ItemDto current = itemService.createItem(2L, itemDtoList.get(2));
        ItemDto updatedNameItem = current.toBuilder().name("updated").build();
        assertEquals(updatedNameItem, itemService.updateItem(2L, updatedNameItem));
        ...
        assertEquals(updatedNameItem.toBuilder().available(false).build(),
                itemService.updateItem(2L, updatedOnlyOneField));

        assertThrows(RuntimeException.class, () -> itemService.updateItem(999L, current));

        //update by not owner
        assertThrows(RuntimeException.class, () -> itemService.updateItem(1L, current));
    }


    @Test
    void getByItemId() {
        ItemDto expected = itemService.createItem(3L, itemDtoList.get(3));
        assertEquals(expected, itemService.getByItemId(expected.getId()));
    }

    @Test
    void getByItemIdWithBookings() throws InterruptedException {
        ItemDto itemDto = itemService.createItem(1L, itemDtoList.get(1));
    ...
    void findByText() {
        int from = 0;
        int size = 10;
        ItemDto itemDto = itemService.createItem(6L, itemDtoList.get(6));
        //search in description and caseInsensitive
        assertTrue(itemService.getItemByComment("6 deSc").contains(itemDto)
                && itemService.getItemByComment("6 dEsc").size() == 1);
        //search in name and caseInsensitive
        itemDto.setDescription("updated");
        itemService.updateItem(6L, itemDto);
        assertTrue(itemService.getItemByComment("item6").contains(itemDto)
                && itemService.getItemByComment("item6").size() == 1);
        //if not found
        assertTrue(itemService.getItemByComment("not exist text").isEmpty());
    ...
    void postCommentForItemFromAuthor() throws InterruptedException {
        ...
        long authorId = 7L;
        ItemDto expected = itemService.createItem(authorId - 1, dto);
        String text = "text";
